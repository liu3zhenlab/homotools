...
# nucmerplot
###########################################################
homomine_nucmerplot <- function(datafile, qrybed="empty", tgtbed="empty",
                                band_col="bisque3", ispdf=F, outpath=".", imageoutfile) {
  ...
  if (qrybed != "empty") { # highlight regions on query
	#chr start end label height strand color
	#10	1000	2488	gene	0.015	+	gray60
	qry_highlight <- read.delim(qrybed, comment.char="#", stringsAsFactors=F, header=F)
  ...
  # target highlights
  highlight_names <- NULL
  if (tgtbed != "empty") { # highlight regions on query
    #chr start end label height strand color
	  #10	1000	2488	gene	gray60	+	0.015
	  target_highlight <- read.delim(tgtbed, comment.char="#", header=F)
  ...
  if (tgtbed != "empty") {
    if (grepl("adjusted.bed$", tgtbed)) {
  		transcript <- gsub(".*\\/", "", tgtbed)
